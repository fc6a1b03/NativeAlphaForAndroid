...
* **极低损耗**：R8 全量压缩，仅保留 arm64-v8a 原生库，release APK 实测约 5.4MB（v2.2.12 引入 CameraX 扫码 +0.65MB；v2.2.22 升级 Kotlin 2.4/Compose 1.12 +0.3MB）；无 GMS 依赖；敏感权限（相机/麦克风/定位）不在启动时索取，仅网页功能触发时按需向用户申请
