...
* **站点分享**：把任意站点打成二维码/深链（`webnative://add`），对方扫一下即进添加向导预填；分享链接 fail-closed 校验（版本门/URL 白名单/userinfo 剥离）
...
* **极低损耗**：R8 全量压缩，仅保留 arm64-v8a 原生库，release APK 实测约 4.4MB（v2.2.11）；无 GMS 依赖；敏感权限（相机/麦克风/定位）不在启动时索取，仅网页功能触发时按需向用户申请
