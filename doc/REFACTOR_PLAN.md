...
> 状态： **全部完成 + UI 全面重构**（阶段 0-8 已执行；UI 全量 Compose 化 + 靛蓝主题落地，待用户模拟器验收）
> 日期：2026-08-19
> 适用分支：dev
...
| D18 | WebView 常驻 | **遵守官方规范不常驻实例**：移除 WebView 热池与 isRestorePage 恢复入口；重进重新加载（内存优先，功能取舍经用户确认） | 用户：UniWebView 官方建议不常驻则遵守，宁可去除该功能 |
| D19 | 长按/小菜单 | **长按完全交还系统**（文字选中复制 100% 正常，零干预）；**小菜单改为双击空白触发**（自实现 300ms 同点双击 + JS caretRangeFromPoint 判空白）。不再用长按判定元素：真实站点空白常误判为文字（历史 bug 根因），且与系统 ActionMode 竞争 | 用户：担心实机长按判定不可靠，改双击空白弹小菜单——不易误触、判断简单 |
| D20 | 双击交互完善 | 输入法拦截用 **Window SOFT_INPUT_STATE_ALWAYS_HIDDEN 同步拦截**（异步 blur 来不及、吞事件破坏触摸分发——均实测排除）；小菜单关闭（onDismiss）收键盘；滚动/拖动/滚轮清除双击状态防误触；滚动后刷新输入框位置缓存 | 用户：双击输入框输入法闪烁、小菜单关闭后键盘弹出、滚动触发小菜单——均实测修复 |
| D21 | 图片双击 | **双击命中媒体元素（img/canvas/svg/video/iframe 祖先链）→ 不弹小菜单**（交还网页双击放大）；输入法拦截仅发生一次（inputIntercepting 标志）+ 双击恢复窗口模式（防抖动循环） | 用户：双击图片放大弹小菜单、输入法出来又收起反复抖动——均实测修复 |
| D22 | 多功能迭代 | ①三点菜单加复制URL/添加快捷键 ②主页搜索过滤（名称+URL模糊，Material 搜索框）③流式优化（LayoutAlgorithm.NORMAL+UTF-8）④多标签会话隔离（Cookie 快照 key=webappId.tabIndex，小菜单新增/切换/删除）⑤错误日志默认展开+每组限10条+清空 ⑥输入法重做（删拦截机制，单击交还 WebView，双击弹菜单收键盘）⑦检查版本更新（GitHub API+DownloadManager 异步+FileProvider 安装） | 用户：7 项需求——发版前深度测试闭环 |
